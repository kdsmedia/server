			...
			<table class="table table-striped- table-bordered table-hover table-checkable" id="dtable">
				...
				<tbody>
					...
					<tr>
						<td>{{$style['id']}}</td>
						<td>{{$style['title']}}</td>
						<td style="{{$style['css']}}">TEST</td>
						<td><a href="/admin/styles/delete/{{$style['id']}}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Delete"><i class="la la-trash"></i></a></td>
					</tr>
					@endforeach
				</tbody>
			</table>
