$(document).ready(function() {
	...
	$('#purposeTip').popover({
        html : true,
		placement: 'top',
        content: '<div class="popover-tip-text">Active referral is considered to be the one who replenished the balance from 10€</div>'
    });
	...
	function timer(remaining) {
		if(!remaining) {
			...
			$('.form_wrapper.group .form_block').html('<div class="form_recharge"><span>Reloading through:</span><div class="form_timeLeft">00:00:00</div></div>');
			return;
		}
		clearInterval(countdown);
		countdown = setInterval(() => {
			if(remaining == 0) {
				remaining == null;
				clearInterval(countdown);
				$('.form_wrapper.group .form_block').html('<div class="form_value">'+ settings.bonus_group_time +' min<div class="form_text">reloading</div></div><span id="spin-wheel-button" class=""><button type="button" class="btn" data-toggle="modal" data-target="#captchaModal">Spin the wheel</button></span>');
				return;
			}
			remaining--;
			var coul = convertTimestamp(remaining);
			$('.form_wrapper.group .form_block').html('<div class="form_recharge"><span>Reloading through:</span><div class="form_timeLeft">'+ coul +'</div></div>');
		}, 1*1000);
	}
