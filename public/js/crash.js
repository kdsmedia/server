$(document).ready(function() {
    ...
    this.publishTime = (res) => {
        $('#chartInfo').text('Start in ' + res.value + 'sec');
    }
    ...
    this.reset = (res) => {
        $('#bets').html('');
        $('#chartInfo').css('color', '#fbfbfc').text('Waiting for bets...');
    ...
    this.parseSlider = (res) => {
        ...
        $('#chartInfo').text(((res.crashed) ? 'Crashed at ' : '') + 'x' + res.float.toFixed(2));
        if(res.crashed) 
        {
            ...
            $('.btn-withdraw span').text('Take it');
        } else {
            if(!window.isCashout && window.withdraw > 0) $('.btn-withdraw span').text('Take it ' + parseFloat(window.bet*parseFloat(res.float.toFixed(2))).toFixed(2));
    ...
    $('.btn-play').click(() => {
        $.ajax({
            ...
            success : res => {
                ...
                {
                    window.bet = res.bet;
                    $('.btn-withdraw span').text('Take it ' + (window.bet).toFixed(2));
