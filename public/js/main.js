...
$(document).ready(function () {
	$('#btnchat').click(function () {
		if(chatHide == 0) {
			chatHide = 1;
			$('.sidebar-container').slideUp(0);
			$('.right-sidebar').css({'background': 'none'});
			$('.wrapper').addClass('chatHide');
			$('#btnchat').addClass('chatClose')
		} else {
			chatHide = 0;
			$('.sidebar-container').slideDown(0);
			$('.right-sidebar').css({'background': '#101a3f', 'position': 'fixed'});
			$('.wrapper').removeClass('chatHide');
			$('#btnchat').removeClass('chatClose')
		}
	});
});
	...
	$('.sendButton').click(function(){
		...
		$.ajax({
			...
			success : function(data) {
                ...
                $('.sendButton').val('Confirm the transfer');
...
$(document).ready(function() {
	//$('body').snowfall({image :"../img/winter/snow/1.png", minSize: 5, maxSize:8, flakeCount: 30});
	initBalance();
	...
	$('#auth').click(function() {
		const login = $('#authlog').val();
		const pass = $('#authpass').val();

		if($.trim(login).length === 0 || $.trim(pass).length === 0) return $.notify({type: 'error', message: 'Fill in all fields'});
		
		$.post('/auth', {login: login, password: pass})
		.then((data) => {
			if(data.error) return $.notify({type: 'error', message: data.message});
			if(data.success) return location.href = '/';
		})
	})
	$('#register').click(function() {
		const login = $('#reglog').val();
		const pass = $('#regpass').val();

		if($.trim(login).length === 0 || $.trim(pass).length === 0) return $.notify({type: 'error', message: 'Fill in all fields'});
		
		$.post('/register', {login: login, password: pass})
		.then((data) => {
			if(data.error) return $.notify({type: 'error', message: data.message});
			if(data.success) return location.href = '/';
		})
	...
	$(document).on('click', '.deposit-wrap .dropdown-item', function() {
		...
		if($(this).data('id') == 'balance') {
			...
			$('#exchange').remove();
			$('#wallet').remove();
			$('.deposit-block').append('<button type="button" class="btn btn-light-gray" id="exchange">Swap</button>');
			type = 'bonus';
		}
	...
	$(document).on('click', '.user-link, .btn-link', function() {
		location.href = '/profile/'+$(this).attr('data-id')
	});
	...
	$(document).on('click', '.select-payment .dropdown-menu .dropdown-item', function() {
		...
		var type = $('.dura .deposit-modal-component .tab.active').data('type');
	...
	$(document).on('click', '.auth-modal-component .btn-tab', function(e) {
        if(!$(this).is('.isActive')) {
            $('.auth-modal-component .tab').removeClass('active');
            $('.auth-modal-component .btn-tab').removeClass('isActive');
            $(this).addClass('isActive');
            $('.auth-modal-component .tab:eq('+$(this).index()+')').addClass('active');
        }   
    });
	
	$(document).on('click', '.bet-component .btn-action, .bet-component .btn-bet-clear, .btn-act', function() {
	...
	$(document).on('submit', '#clan', function(e) {
		$.post('/clans/create', {
			name: $('#clanName').val(),
			avatar: $('#clanAvatar').val(),
			points: $('#clanPoints').val()
		})
		.then(res => {
			if(res.type == 'success') {
				setTimeout(() => {
	...
	$('#register-checkbox').click(function() {
        $('#auth-checkbox').attr('checked', 'checked');
        if($(this).prop('checked') == true){
            $('#register').removeAttr('disabled');
        } else {
            $('#register').attr('disabled', 'false');
            $('#register-checkbox').removeAttr('checked');
        }
    });
	...
	function calcSum(system) {
		...
		$('.com-row span').html((perc != 0 ? perc + '%' : 'No')+(com != 0 ? '+' + com + '€' : ''));
	...
	socket
	.on('updateKing', function (data) {
		$('#kingBets').prepend('\
							<tr>\
								<td class="username">\
									<button type="button" class="btn btn-link" data-id="' +data.bet.unique_id+ '">\
										<span class="sanitize-user">\
											<div class="sanitize-avatar"><img src="' +data.bet.avatar+ '" alt=""></div>\
											<span class="sanitize-name" style="' +data.bet.style+ '">' +data.bet.login+ '</span>\
										</span>\
									</button>\
								</td>\
		...
		$('#kingBets').children().slice(10).remove();
		$('.king_bets').html('');
		data.players.forEach(e => {
			$('.king_bets').append('<img src="' +e.avatar+ '">')
		})
		clearTimeout(tckfj)
		tckfj = setTimeout(() => {
			$('.king_avatar').removeClass('newBet')
		}, 250)
		$('.king_bets').prepend('<img class="currentBet" src="' +data.bet.avatar+ '">');
		$('.king_avatar').addClass('newBet').html('<img src="' +data.bet.avatar+ '">');
		$('.king_bank').html(data.bank.toFixed(2)+' <svg class="icon icon-coin"><use xlink:href="/img/symbols.svg#icon-coin"></use></svg>');
	})
	.on('kingTimer', function (data) {
		if(data.type == 'timer') {
			$('.king_timer').html(data.data.timer);
		}
	})
	.on('kingWinner', function(data) {
		$('.king_avatar').removeClass('newBet');
		$('.king_avatar').addClass('winner');
	})
	.on('kingClear', function (data) {
		$('.king_bank').html('0.00 <svg class="icon icon-coin"><use xlink:href="/img/symbols.svg#icon-coin"></use></svg>');
		$('.king_timer').html('00:10');
		$('.king_avatar img, .king_bets img').fadeOut();
		$('.king_avatar').removeClass('winner');
	})
	...
	.on('chat', function (data) {
        ...
        if(data.admin) name = '<span class="admin-badge isAdmin" data-toggle="tooltip" data-placement="top" title="Administrator"><span class=""><svg class="icon icon-a"><use xlink:href="/img/symbols.svg#icon-a"></use></svg></span></span> Administrator ';
        if(data.moder) name = '<span class="admin-badge isModerator" data-toggle="tooltip" data-placement="top" title="Moderator"><span class=""><svg class="icon icon-m"><use xlink:href="/img/symbols.svg#icon-m"></use></svg></span></span> ' + data.username;
		...
		$('.chat-conversation-inner').append(
			...
			'<span class="sanitize-name">' + data.rank +
			'<span class="sanitize-text" style="'+data.userStyle+'">' + name + '<span>&nbsp;</span></span>' +
			'</span>' +
			'</button>' +
	...
	.on('ban_message', function(data) {
		if(USER_ID == data.unique_id && data.ban) {
			...
			$('.chat').append('<div class="chat-ban-block"><div class="title">Chat is blocked!</div><button type="button" class="btn btn-light unbanMe"><span>Unlock ( -50 <svg class="icon icon-coin balance"><use xlink:href="/img/symbols.svg#icon-coin"></use></svg> )</span></button></div>');
		}
		if(USER_ID == data.unique_id && !data.ban) {
	...
	.on('giveaway', function(res) {
		if(res.type == 'new') {
			...
			var seconds = res.data.time_to,
				...
				time = hour +':'+ min +':'+ sec,
				html = '',
				group_sub = (res.data.group_sub ? '<div class="faucet-sm-text">• To be subscribed to our <a href="'+ settings.vk_url +'" target="_blank">VK group</a>.</div>' : ''),
				min_dep = (res.data.min_dep ? '<div class="faucet-sm-text">• Top up the account witch the amount of '+ parseFloat(res.data.min_dep).toFixed(2) +'€. for the current day.</div>' : ''),
				ruleBlock = (res.data.group_sub || res.data.min_dep ? '<div class="give-btn-block nowGive"><div class="faucet-reload"><div class="faucet-cd">To participate, you need to:</div><div class="text-left">'+ group_sub +''+ min_dep +'</div></div></div>' : '');
				html += '<div class="faucet-modal-give" id="gv_'+ res.data.id +'"><div class="give-btn-block"><div class="faucet-reload"><div class="faucet-cd">Handout #'+ res.data.id +'</div></div></div><div class="row"><div class="col-6"><div class="bank-text">Amount:</div><div class="bank-amount">'+ parseFloat(res.data.sum).toFixed(2) +' <svg class="icon icon-coin '+ res.data.type +'"><use xlink:href="/img/symbols.svg#icon-coin"></use></svg></div></div><div class="col-6"><div class="date-text">Before the start of the:</div><div class="date-of">'+ time +'</div></div></div>'+ ruleBlock +'<div class="give-btn-block nowGive"><button type="button" class="btn btn-green joinGiveaway" data-id="'+ res.data.id +'"><span>Participate</span></button><div class="faucet-sm-text">Participate: <span class="total_users">0</span></div></div><div class="give-btn-block"><div class="faucet-reload"><div class="faucet-cd">Winner:</div><div class="winnerGive">No</div></div></div></div>'
		...
		if(res.type == 'timer') {
			if(res.status == 1) $('#gv_' + res.id + ' .date-of').html('Carried out');
			else $('#gv_' + res.id + ' .date-of').html(res.hour+':'+res.min+':'+res.sec);
		}
		if(res.type == 'winner') {
			$('#gv_' + res.id).addClass('doneGive');
			$('#gv_' + res.id + ' .date-of').html('Carried out');
		...
		if(res.type == 'delete') {
			$('#gv_' + res.id).remove();
			if($('.faucet-modal-give').length == 0) $('.gv-list').html('<div class="faucet-modal-give giveNone"><div class="give-btn-block"><div class="faucet-reload"><div class="faucet-cd">There are no giveaways.</div></div></div></div>');
		}
	});
...
$(function() {
    ...
    const accordionCloseIcon = $('.accordion-header__switch');

    $(accordionHeader).click(function () {
        $(accordionContent).slideToggle();
        $(accordionCloseIcon).toggleClass('accordion-header__switch-close');
    })
});