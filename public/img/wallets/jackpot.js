$(document).ready(function() {
    ...
    socket.on('jackpot', r => {
        ...
        if(r.type == 'newGame') {
			if(room == r.room) {
				...
				chart.data.datasets[0].backgroundColor = ['#202a51'];
				chart.update();
			}
    ...
    function JackpotParse(res) {
        ...
        res.bets.forEach(bet => {
            bets += '<tr><td class="username"> <button type="button" class="btn btn-link" data-id="'+bet.user.id+'"> <span class="sanitize-user"><div class="sanitize-avatar"><img src="'+bet.user.avatar+'" alt="" style="border: 2px solid #'+bet.bet.color+';"></div> <span class="sanitize-name">'+bet.user.username+'</span> </span> </button></td><td><div class="bet-number"> <span class="bet-wrap"> <span>'+bet.bet.amount+'</span> <svg class="icon icon-coin '+bet.bet.balance+'"> <use xlink:href="/img/symbols.svg#icon-coin"></use> </svg> </span></div></td><td>'+bet.bet.chance+'%</td><td><div class="bet-number rtl"> <span class="bet-wrap"> <span>'+bet.bet.from+' - '+bet.bet.to+'</span> <svg class="icon"> <use xlink:href="/img/symbols.svg#icon-ticket"></use> </svg> </span></div></td></tr>';
        });
        $('#bets').html(bets);

        let chances = '';
        res.chances.forEach(chance => {
            chances += '<div class="item" data-toggle="tooltip" data-placement="top" title="'+chance.user.username+'"><div class="user"><img src="'+chance.user.avatar+'" alt="" style="border: 2px solid #'+chance.color+';"></div><div class="hit">'+chance.chance+'%</div></div>';
		...
		if($('.game-tooltip.won').length != 0) {
			$('.game-tooltip.won').removeClass('isActive');
			$('.game-tooltip.won.'+res.room).removeClass('isActive');
            $('.game-tooltip.won.'+res.room).remove();
		}
