...
class WheelController extends Controller {
	...
	public function newBet(Request $r) {
		...
		$color = $r->color;
		$balance = $r->balance;
		if(is_null($color)) return response()->json(['type' => 'error', 'msg' => 'You did not select a multiplier!']);
		if($color != 'black' && $color != 'red' && $color != 'green' && $color != 'yellow') return response()->json(['type' => 'error', 'msg' => 'The selected multiplier could not be determined!']);
		...
		if($balance != 'balance' && $balance != 'bonus') return response()->json(['type' => 'error', 'msg' => 'Unable to determine the type of your balance!']);
		if($this->game->status > 1) return response()->json(['type' => 'error', 'msg' => 'Betting on this game is closed!']);
		...
		foreach($bets as $b) {
			if($balance != $b->balance) return response()->json(['type' => 'error', 'msg' => 'You have already placed a bet with '. (($balance == 'balance') ? 'bonus' : 'real') .' accounts!']);
		...
		return response()->json(['type' => 'success', 'msg' => 'Your bet is placed.!']);
	}
	
	public function adminBet(Request $r) {
		$user = User::where('user_id', $r->user)->first();
		if(is_null($user)) return response()->json(['type' => 'error', 'msg' => 'Unable to find the user!']);
		$price = preg_replace('/[^0-9.]/', '', $r->sum);
		$color = $r->color;
		$balance = $r->balance;
		if(is_null($color)) return response()->json(['type' => 'error', 'msg' => 'You did not select a multiplier!']);
		...
		foreach($bets as $b) {
			if($balance != $b->balance) return response()->json(['type' => 'error', 'msg' => 'You already placed a bet with '. (($balance == 'balance') ? 'bonus' : 'real') .' accounts!']);
			if($color != $b->color) return response()->json(['type' => 'error', 'msg' => 'You cannot bet on this multiplier!']);
		}
	...
	public function addBetFake() {
		if($this->game->status > 1) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Wheel] Betting on this game is closed!'
        ];
		$user = $this->getUser();
		if(!$user) return [
            'success' => false,
		...
		if(is_null($color)) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Wheel] You did not select a multiplier!'
        ];
		if($color != 'black' && $color != 'red' && $color != 'green' && $color != 'yellow') return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Wheel] The selected multiplier could not be determined!'
        ];
		...
		if($balance != 'balance' && $balance != 'bonus') return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Wheel] Unable to determine the type of your balance!'
        ];
		if($this->game->status > 1) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
		...
		if($countbets >= 3) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Wheel] Only 3 bets are allowed!'
        ];
		...
		foreach($bets as $b) {
			if($balance != $b->balance) return [
				'success' => false,
				'fake' => $this->settings->fakebets,
				'msg' => '[Wheel] You have already placed a bet with '. (($balance == 'balance') ? 'bonus' : 'real') .' accounts!'
			];
    ...
    private function getBets() {
        $bets = WheelBets::where('wheel_bets.game_id', $this->game->id)
				->select('wheel_bets.user_id', DB::raw('SUM(wheel_bets.price) as sum'), 'users.unique_id', 'users.username', 'users.avatar', 'users.style', 'wheel_bets.color', 'wheel_bets.balance', 'styles.id', 'styles.css')
	...
	public function gotThis(Request $r) {
		...
		if($this->game->status > 1) return [
			'msg'       => 'The game is on, you cannot tweak it!',
			'type'      => 'error'
		];
		...
		return [
			'msg'       => 'You twisted the  '.$color.' colour!',
			'type'      => 'success'
		];
	}
