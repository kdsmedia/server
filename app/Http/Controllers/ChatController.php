...
class ChatController extends Controller
{
    ...
    public static function chat()
    {
        ...
        foreach ($value as $key => $newchat[$i]) {
            ...
            $returnValue[$i] = [
            	'rank' => (isset($value2[$i]['rank'])) ? $value2[$i]['rank'] : 0,
            	'userStyle' => (isset($value2[$i]['userStyle'])) ? $value2[$i]['userStyle'] : 0,
	...
	public function ban(Request $r) {
		...
		if(is_null($user)) return response()->json(['success' => false, 'msg' => 'User not found!', 'type' => 'error']);
		if(!is_numeric($r->get('time')) || $r->get('time') <= 0) return response()->json(['success' => false, 'msg' => 'You have entered an invalid value for the "Time" field!', 'type' => 'error']);
		if(!is_null($user->banchat)) return response()->json(['success' => false, 'msg' => 'The user has already been blocked!', 'type' => 'info']);
		if($user->unique_id == $this->user->unique_id) return response()->json(['success' => false, 'msg' => 'You can not block yourself!', 'type' => 'info']);
		...
		$returnValue = ['unique_id' => $unique_id, 'avatar' => $avatar, 'time2' => Carbon::now()->getTimestamp(), 'time' => $time, 'messages' => '<span style="color: #4986f5;">User "'.$user->username.'" locked in a chat room for '.floatval($r->get('time')).' min.'. ($r->get('reason') ? ' Reason: '.htmlspecialchars($r->get('reason')).'.' : '') .'</span>', 'username' => $username, 'ban' => $ban, 'admin' => $admin, 'moder' => $moder, 'youtuber' => $youtuber];
		...
		return response()->json(['success' => true, 'msg' => 'User '. $user->username .' locked in a chat room for '. $r->get('time') .' min!', 'type' => 'success']);
	}
	...
	public function unban(Request $r) {
		...
		if($user->unique_id == $this->user->unique_id) return response()->json(['success' => false, 'msg' => 'You can not unlock yourself!', 'type' => 'info']);
		...
		$returnValue = ['unique_id' => $unique_id, 'avatar' => $avatar, 'time2' => Carbon::now()->getTimestamp(), 'time' => $time, 'messages' => '<span style="color: #4986f5;">User "'.$user->username.'" unblocked from chat.</span>', 'username' => $username, 'ban' => $ban, 'admin' => $admin, 'moder' => $moder, 'youtuber' => $youtuber];
		$returnBan = ['unique_id' => $user->unique_id, 'username' => $banusername, 'ban' => 0];
		$this->redis->publish(self::BAN_CHANNEL, json_encode($returnBan));
		$this->redis->rpush(self::CHAT_CHANNEL, json_encode($returnValue));
		$this->redis->publish(self::NEW_MSG_CHANNEL, json_encode($returnValue));
		
		return response()->json(['success' => true, 'msg' => 'User '. $user->username .' unblocked from chat!', 'type' => 'success']);
	}
	
	public function clear() {
		$this->redis->del(self::CHAT_CHANNEL);
		$this->redis->publish(self::CLEAR, 1);
		return response()->json(['success' => true, 'msg' => 'You cleared the chat room!', 'type' => 'success']);
	}
	...
	public function unBanFromUser() {
		...
		return response()->json(['msg' => 'The users are unbanned', 'status' => 'success']);
	}

    public function add_message(Request $request) {
		if(\Cache::has('action.user.' . $this->user->id)) return response()->json(['message' => 'You are sending too many messages!', 'status' => 'error']);
        ...
        if(!$this->user->is_admin && !$this->user->is_moder && !$this->user->is_youtuber) {
            if($this->settings->chat_dep != 0) if($dep < $this->settings->chat_dep) {
                return response()->json(['message' => 'In order to write in the chat room, you need to top up your account by '.$this->settings->chat_dep.' EUR!', 'status' => 'error']);
            }
        }
		...
		$userStyle = "";
		$rankIcon = "";
		if($this->user->style != 0 and !$admin) {
			$style = Styles::where('id', $this->user->style)->first();
			$userStyle = $style->css;
		}
		if($this->user->rank != 0 and !$admin) {
			$rank = Ranks::where('id', $this->user->rank)->first();
			$rankIcon = '<img style="height: 15px; margin: 0 3px 0 0;" data-toggle="tooltip" data-placement="top" title="" src="'.$rank->icon.'" data-original-title="'.$rank->title.'">';
		}
        ...
        $returnValue = ['rank' => $rankIcon, 'userStyle' => $userStyle, 'unique_id' => $unique_id, 'avatar' => $avatar, 'time2' => Carbon::now()->getTimestamp(), 'time' => $time, 'messages' => $messages, 'username' => $username, 'ban' => $ban, 'admin' => $admin, 'moder' => $this->user->is_moder, 'youtuber' => $this->user->is_youtuber];
        $this->redis->rpush(self::CHAT_CHANNEL, json_encode($returnValue));
        $this->redis->publish(self::NEW_MSG_CHANNEL, json_encode($returnValue));
