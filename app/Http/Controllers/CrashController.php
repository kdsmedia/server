...
class CrashController extends Controller {
    ...
    public function newBet(Request $r) {
        ...
        if(floatval($r->get('withdraw')) > 10000 && floatval($r->get('withdraw')) < 0) return [
            'success' => false,
            'msg' => 'You have entered an invalid value for auto withdrawal!'
        ];
        ...
        try {
			...
			if($balType == 'balance') {
				DB::table('users')->where('id', $this->user->id)->update([
                    ...
                    'bsum' => $this->user->bsum+floatval($r->get('bet'))
				]);
            ...
            return [
                'success' => true,
                'msg' => 'Your bet has been approved!',
    ...
    public function addBetFake() {
        ...
        if($this->game->status > 0) return [
            ...
            'msg' => '[Crash] Bets for this round are closed!'
        ];
		if($countBet == 5) return [
            'success' => false,
            'fake' => $this->settings->fake,
            'msg' => '[Crash] This user is already engaged!'
        ];
        ...
        try {
            ...
            return [
                'success' => true,
				'fake' => $this->settings->fake,
                'msg' => '[Crash] Your bet has been approved!'
            ];
        } catch(Exception $e) {
            DB::rollback();
            return [
    ...
    public function Cashout() {
        ...
        if($this->game->status == 2) return [
            'success' => false,
            'msg' => 'This round has already been closed!'
        ];
        ...
        try {
            ...
            if($cashout == 0) {
                ...
                return [
                    'success' => false,
                    'msg' => 'You cannot withdraw your bet! The round has not yet started or has already ended...'
                ];
            }
            ...
            return [
                'success' => true,
                'msg' => 'Congratulations! You have multiplied your bet by x'. $float .' and got '. round($bet->price*$float, 2) .' coins!'
            ];
	...
	public function gotThis(Request $r) {
		...
		if(!$multiplier) return [
			'msg'       => 'Failed to get a multiplier!',
			'type'      => 'error'
		];
		...
		return [
			'msg'       => 'You have twisted it on x'.$multiplier.'!',
			'type'      => 'success'
		];
	}
}