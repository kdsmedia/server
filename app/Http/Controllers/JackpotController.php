...
class JackpotController extends Controller {
	...
	public function newGame(Request $r) {
		...
		if(is_null($room)) return response()->json([
			'success' => false,
			'msg' => 'Could not find the room you wish to bid in!'
		]);
	...
	public function newBet(Request $r) {
		...
		if(is_null($room)) return [
			'success' => false,
			'msg' => 'Could not find the room you wish to bet in!'
		];
		...
		if($game->status > 1) return [
			'success' => false,
			'msg' => 'The stakes in this game are closed!'
		];
		...
		if($userbets->count() >= $room->bets) return [
			'success' => false,
			'msg' => 'You cannot do more '.$room->bets.' bets per game!'
		];
			
		if(floatval($r->get('amount')) < $room->min && $balance == 'balance' || $balance == 'bonus' && ($r->get('amount') / $this->settings->exchange_curs) < $room->min) return [
			'success' => false,
			'msg' => 'Minimum bet amount '.$room->min.' coin(s)!'
		];
		if(floatval($r->get('amount')) > ($room->max - $userbets->sum('sum')) && $balance == 'balance' || $balance == 'bonus' && ($r->get('amount') / $this->settings->exchange_curs) > ($room->max - $userbets->sum('sum'))) return [
			'success' => false,
			'msg' => $room->max - $userbets->sum('sum')
		];
		...
		try {
			...
			if($betType > 0) {
				...
				return [
					'success' => false,
					'msg' => 'You already placed a bet with'. (($balance == 'balance') ? 'bonus' : 'real') .' accounts!'
				];
			}
			if($balance != 'balance' && $balance != 'bonus') {
				...
				return [
					'success' => false,
					'msg' => 'Unable to determine the type of your balance!'
				];
			}
	...
	public function adminBet(Request $r) {
		...
		if(is_null($game)) return [
			'success' => false,
			'msg' => 'Failed to find the game in the room '.$room->name
		];
		...
		try {
			...
			if(floatval($r->get('amount')) > ($room->max - $userbets->sum('sum'))) {
				...
				return [
					'success' => false,
					'msg' => 'You cannot bet more '.$room->max.' coin(s) per game!'
				];
			}
	...
	public function addBetFake() {
		...
		if(is_null($room)) return [
			...
			'msg' => '[ROOM #'.$room->name.'] Could not find the room you wish to bid in!'
		];
		if(is_null($user)) return [
			'success' => false,
            'fake' => $this->settings->fakebets,
			'msg' => '[ROOM #'.$room->name.'] Unable to find the user!'
		];
		if(is_null($game)) return [
			'success' => false,
            'fake' => $this->settings->fakebets,
			'msg' => '[ROOM #'.$room->name.'] Could not find the game!'
		];

		if($game->status > 1) return [
			'success' => false,
            'fake' => $this->settings->fakebets,
			'msg' => '[ROOM #'.$room->name.'] The stakes in this game are closed!'
		];
		...
		try {
			...
			if($userbets->count() >= $room->bets) {
				...
				return [
					'success' => false,
            		'fake' => $this->settings->fakebets,
					'msg' => '[ROOM #'.$room->name.'] You cannot do more '.$room->bets.' bets per game!'
				];
			}
			...
			if(floatval($sum) < $room->min) {
				...
				return [
					'success' => false,
            		'fake' => $this->settings->fakebets,
					'msg' => '[ROOM #'.$room->name.'] Minimum bet amount '.$room->min.' coin(s)!'
				];
			}
			...
			if(floatval($sum) > ($room->max - $userbets->sum('sum'))) {
				...
				return [
					'success' => false,
            		'fake' => $this->settings->fakebets,
					'msg' => '[ROOM #'.$room->name.'] You cannot bet more '.$room->max.' coin(s) per game!'
				];
			}
			...
			if($betType > 0) {
				...
				return [
					'success' => false,
            		'fake' => $this->settings->fakebets,
					'msg' => '[ROOM #'.$room->name.'] You already placed a bet with '. (($balance == 'balance') ? 'bonus' : 'real') .' accounts!'
				];
			}
			if($balance != 'balance' && $balance != 'bonus') {
				DB::rollback();
				return [
					'success' => false,
            		'fake' => $this->settings->fakebets,
					'msg' => '[ROOM #'.$room->name.'] Unable to determine the type of your balance!'
				];
			}
		...
		return [
			'success' => true,
			'fake' => $this->settings->fakebets,
			'msg' => '[ROOM #'.$room->name.'] Your bet Is in play!'
		];
	}
	...
	public function getSlider(Request $r) {
		...
		if(!$room) return [
			'success' => false,
			'msg' => 'Failed to find a room '.$r->get('room')
		];
		...
		if($game->id != $r->get('game')) return [
			'success' => false,
			'msg' => 'Found the game #'.$game->id.'. It is not #'.$r->get('game')
		];
		...
		if(is_null($winnerBet)) return [
			'success' => false,
			'msg' => 'Failed to find a winning bet! Repeat..',
			'retry' => true
		];
	...
	public function gotThis(Request $r) {
		...
		if(!$userid) return [
			'msg'       => 'Failed to retrieve player id!',
			'type'      => 'error'
		];
		
		if(is_null($bets)) return [
			'msg'       => 'This player did not place a bet!',
			'type'      => 'error'
		];
		...
		return [
			'msg'       => 'You tampered with a player '.$user->username.' in the mode of Jackpot!',
			'type'      => 'success'
		];
	}
