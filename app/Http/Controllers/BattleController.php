...
class BattleController extends Controller {
	...
	private function getBets() {
        $bets = BattleBets::where('battle_bets.game_id', $this->game->id)
				->select('battle_bets.user_id', DB::raw('SUM(battle_bets.price) as price'), 'users.username', 'users.avatar', 'users.unique_id', 'battle_bets.color', 'battle_bets.balType', 'styles.id', 'styles.css')
				->join('users', 'users.id', '=', 'battle_bets.user_id')
				->join('styles', 'users.style', '=', 'styles.id')
	...
	public function newBet(Request $r) {
		...
		if(is_null($color) || $color != 'red' && $color != 'blue') return response()->json(['msg' => 'Colour not found', 'type' => 'error']);
		if(is_null($sum)) return response()->json(['msg' => 'You have entered an incorrect amount', 'type' => 'error']);
		if($sum < $this->settings->battle_min_bet || $balType == 'bonus' && ($sum / $this->settings->exchange_curs) < $this->settings->battle_min_bet) return response()->json(['msg' => 'Minimum bet amount '.$this->settings->battle_min_bet.' coins!', 'type' => 'error']);
		if($sum > $this->settings->battle_max_bet || $balType == 'bonus' && ($sum / $this->settings->exchange_curs) > $this->settings->battle_max_bet) return response()->json(['msg' => 'Maximum bet amount '.$this->settings->battle_max_bet.' coins!', 'type' => 'error']);
		...
		if($countbets >= 3) return response()->json(['msg' => 'Only 3 bets are allowed!', 'type' => 'error']);
		
		foreach($bets as $b) {
			if($balType != $b->balType) return response()->json(['type' => 'error', 'msg' => 'You have already placed a bet with '. (($balType == 'balance') ? 'bonus' : 'real') .' accounts!']);
			if($color != $b->color) return response()->json(['type' => 'error', 'msg' => 'You have already bet on a different colour!']);
		}
		...
		return response()->json(['msg' => 'Your bid has been accepted!', 'type' => 'success']);
	}
	...
	public function adminBet(Request $r) {
		...
		if(is_null($color) || $color != 'red' && $color != 'blue') return response()->json(['msg' => 'Colour not found', 'type' => 'error']);
		if(is_null($sum)) return response()->json(['msg' => 'You have entered an incorrect amount', 'type' => 'error']);
		if($sum < $this->settings->battle_min_bet) return response()->json(['msg' => 'Minimum bet amount '.$this->settings->battle_min_bet.' coins!', 'type' => 'error']);
		if($sum > $this->settings->battle_max_bet) return response()->json(['msg' => 'Maximum bet amount '.$this->settings->battle_max_bet.' coins!', 'type' => 'error']);
		if($this->game->status == Battle::STATUS_PRE_FINISH || $this->game->status == Battle::STATUS_FINISHED) return response()->json(['msg' => 'The game has already started or ended!', 'type' => 'error']);
	...
	public function addBetFake() {
		...
		if($balType != 'balance' && $balType != 'bonus') return [
            ...
            'msg' => '[Battle] Unable to determine the type of your balance!'
        ];
		if(is_null($color) || $color != 'red' && $color != 'blue') return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Battle] Colour not found'
        ];
		...
		if($sum < $this->settings->battle_min_bet) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Battle] Minimum bet amount '.$this->settings->battle_min_bet.' coins!'
        ];
		if($sum > $this->settings->battle_max_bet) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Battle] Maximum bet amount '.$this->settings->battle_max_bet.' coins!'
        ];
		...
		if($countbets >= 3) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Battle] Only 3 bets are allowed!'
        ];
		...
		foreach($bets as $b) {
			if($balType != $b->balType) return [
				'success' => false,
				'fake' => $this->settings->fakebets,
				'msg' => '[Battle] You have already placed a bet with '. (($balType == 'balance') ? 'bonus' : 'real') .' account!'
			];
			if($color != $b->color) return [
				'success' => false,
				'fake' => $this->settings->fakebets,
				'msg' => '[Battle] You have already bet on a different colour!'
			];
		}
		...
		return [
			'success' => true,
			'fake' => $this->settings->fakebets,
			'msg' => '[Battle] Your bid has been accepted.!'
		];
	}
